...
class KlarnaValidationControllerTest extends ModuleUnitTestCase
{
    ...
    public function testInit($requestBody, $isValid, $errors, $eRes)
    {
        ...
        $validationController = $this->getMockBuilder(KlarnaValidationController::class)
            ->setMethods(['getRequestBody', 'logKlarnaData', 'getValidator', 'setValidResponseHeader'])->getMock();
    ...
    public function testInit_errorsAndLogs()
    {
        ...
        $validationController = $this->getMockBuilder(KlarnaValidationController::class)
            ->setMethods(['getRequestBody', 'getValidator'])->getMock();
        $validationController->expects($this->once())
            ->method('getRequestBody')
            ->willReturn($requestBody);
        $validationController->expects($this->once())
