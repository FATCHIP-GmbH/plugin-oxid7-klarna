...
class KlarnaCheckoutClientTest extends ModuleUnitTestCase
{
    ...
    public function testCreateOrUpdateOrder()
    {
        ...
        $checkoutClient->expects($this->once())->method('formatOrderData')
            ->willReturn(json_encode(['dummy' => 'data']));
        ...
        $exceptionMock = $this->getMockBuilder(KlarnaCheckoutClient::class)
            ...
            ->getMock();
        $exceptionMock->expects($this->at(0))->method('postOrder')
            ->will($this->throwException(new KlarnaOrderNotFoundException()));
