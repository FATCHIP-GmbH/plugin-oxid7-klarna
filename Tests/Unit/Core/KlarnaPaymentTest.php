...
class KlarnaPaymentTest extends ModuleUnitTestCase
{
    ...
    public function testGetOrderData_AddOptions()
    {
        ...
        $this->setModuleConfVar('aKlarnaDesign', [], 'arr');
        $this->setModuleConfVar('aKlarnaDesignKP', [], 'arr');
        $oKlarnaOrder = new  KlarnaPayment($oBasket, $oUser, []);
    ...
    public function testValidateClientToken()
    {
        ...
        $this->assertEquals(
            ['TCKLARNA_INVALID_CLIENT_TOKEN'],
            $this->getProtectedClassProperty($oKlarnaOrder, 'errors')
        );
    ...
    public function testValidateKlarnaUserData()
    {
        ...
        $this->assertEquals(
            'Payment with this Klarna payment method is currently available only for companies.',
            $errors[0]
        );
