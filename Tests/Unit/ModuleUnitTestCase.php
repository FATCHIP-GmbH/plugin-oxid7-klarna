...
class ModuleUnitTestCase extends UnitTestCase
{
    ...
    {
        $this->setModuleConfVar('sKlarnaActiveMode', $mode);
    }
    ...
    public function setModuleConfVar($name, $value, $type = 'str')
    {
        ...
        if($type == "str") {
            $moduleSettingService->saveString($name, $value, 'tcklarna');
        } elseif ($type == "bool") {
            $moduleSettingService->saveBoolean($name, $value, 'tcklarna');
        } elseif ($type == "arr") {
            $moduleSettingService->saveCollection($name, $value, 'tcklarna');
        } elseif ($type == "int") {
            $moduleSettingService->saveInteger($name, $value, 'tcklarna');
        } elseif ($type == "float") {
            $moduleSettingService->saveFloat($name, $value, 'tcklarna');
        }
    }
    ...
    protected function getModuleTestDir()
    {
        ...
        foreach($this->testConfig->getPartialModulePaths() as $modulePartialPath){
            if(strpos($modulePartialPath, $this->moduleName)){
                return $oFacts->getShopRootPath() . '/' . $modulePartialPath .'/Tests/';
            }
        }
    }
...
//        $oBasket->setBasketUser($oUser);
    ...
    public function prepareKlarnaOrder($paymentId = '')
    {
