...
window.klarnaAsyncCallback = function () {
    ...
    var $sbmButton = $(".sticky-md-top [type=button]");
    ...
    (function initKlarnaPayment() {
        ...
        if ($form.attr('id') === 'payment') {
            ...
            $kpRadio.each(function () {
                ...
                if (this.checked) {
                    ...
                    $kpRadio.active = this;
                    //if some KP Payment is checked on loading, set the onclickEvent of submitBtn to false
                    if(!$sbmButton.attr('onclick').includes('false;')) {
                        $sbmButton.attr('onclick','false;');
                    }
                }
            });
        }
        ...
        if ($form.attr('id') === 'payment') {
            $kpRadio.click(function () {
                //if some KP Payment is checked, set the onclickEvent of submitBtn to false
                if(!$sbmButton.attr('onclick').includes('false;')) {
                    $sbmButton.attr('onclick','false;');
                }
                if (!this.hasError && tcKlarnaClientToken) {
            ...
            $otherRadio.click(function () {
                delete $kpRadio.active;
                $('.kp-method').hide(600);
                //if other payment selected, set the submitBtn to default onclickEvent
                if(!$sbmButton.attr('onclick').includes('document.getElementById(\'payment\').submit();')) {
                    $sbmButton.attr('onclick','document.getElementById(\'payment\').submit();');
                }
            });
        ...
        // Set OrderBtn from last checkout step to false, if a tcKlarnaClientToken is available
        $bodyClassOrder = document.querySelector('body.cl-order');
        if(tcKlarnaClientToken && $bodyClassOrder) {
            $orderBtn = document.querySelector('button.btn-highlight');
            if($orderBtn.getAttribute('onclick') == "document.getElementById('orderConfirmAgbBottom').submit();") {
                $orderBtn.setAttribute('onclick', 'false;')
            }
        }
