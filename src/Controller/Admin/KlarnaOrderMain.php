...
class KlarnaOrderMain extends KlarnaOrderMain_parent
{
    ...
    public function sendorder()
    {
        ...
        if ($cancelled) {
            $this->addTplParam(
                'sErrorMessage',
                Registry::getLang()->translateString("TCKLARNA_CAPUTRE_FAIL_ORDER_CANCELLED")
            );
        ...
        if ($inSync && $this->klarnaOrderData['remaining_authorized_amount'] != 0) {
            ...
            try {
                ...
                $response = $oOrder->captureKlarnaOrder(
                    $data,
                    $oOrder->getFieldData(
                        'tcklarna_orderid'
                    ),
    ...
    public function save()
    {
        ...
        if ($discountChanged) {
            if ($captured) {
                $this->addTplParam(
                    'sErrorMessage',
                    Registry::getLang()->translateString('TCKLARNA_ORDER_UPDATE_CANT_BE_SENT_TO_KLARNA')
                );
    ...
    public function isKlarnaOrder()
    {
        ...
        if ($this->getEditObject() && stripos(
                $this->getEditObject()->getFieldData('oxpaymenttype'),
                'klarna_'
            ) !== false) {
            $blActive = true;
        }
    ...
    protected function handleResponseOrderData($oOrder)
    {
        if (is_array($this->klarnaOrderData)) {
            ...
            if ($this->klarnaOrderData['status'] === 'CANCELLED') {
                ...
                if ($this->klarnaOrderData['order_amount'] != KlarnaUtils::parseFloatAsInt(
                        $oOrder->getTotalOrderSum() * 100
                    )
                    || !$this->isCaptureInSync($this->klarnaOrderData)) {
                    $oOrder->oxorder__tcklarna_sync = new Field(0);
