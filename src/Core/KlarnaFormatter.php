...
class KlarnaFormatter extends Base
{
    ...
    static $aMaleSalutations = [
        'Mr' => ['gb', 'other'],
        'Herr' => ['de', 'at', 'ch'],
        'Dhr.' => ['nl'],
    ];

    static $aFemaleSalutations = [
        'Ms' => ['gb', 'other'],
        'Mrs' => ['gb'],
        'Miss' => ['gb'],
        'Frau' => ['de', 'at', 'ch'],
    ...
    public static function klarnaToOxidAddress($aCheckoutData, $sKey)
    {
        ...
        foreach (self::$aFieldMapper as $oxName => $klarnaName) {
            if ($klarnaName === 'street_address') {
                ...
                if ($klarnaName === 'title') {
                    $aUserData[$sTable . $oxName] = self::formatSalutation(
                        $aAddressData[$klarnaName],
                        strtolower($countryISO)
                    );
                } else {
    ...
    protected static function validateInstance(&$oxObject)
    {
        if ($oxObject instanceof User) {
            ...
            if ($oxObject instanceof Address) {
                $sTable = 'oxaddress__';
    ...
    public static function formatSalutation($title, $sCountryISO)
    {
        ...
        if (!in_array(strtolower($sCountryISO), ['gb', 'de', 'at', 'ch', 'nl'])) {
            $sCountryISO = 'other';
        }
    ...
    public static function getFormattedUserAddresses($_oUser)
    {
        ...
        $oQueryBuilder
            ...
            ->setParameter(':oxuserid', $_oUser->getId());
        $aUserResults = $oQueryBuilder->execute();
        $aUserResults = $aUserResults->fetchAllAssociative();

        if (!is_array($aUserResults) || empty($aUserResults)) {
            return false;
        }

        $formattedResults = [];
        foreach ($aUserResults as $aData) {
            $formattedResults[$aData['oxid']] =
                $aData['oxfname'] . ' ' .
                $aData['oxlname'] . ', ' .
                $aData['oxstreet'] . ' ' .
                $aData['oxstreetnr'] . ', ' .
