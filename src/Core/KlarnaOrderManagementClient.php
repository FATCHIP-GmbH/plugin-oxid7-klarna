...
class KlarnaOrderManagementClient extends KlarnaClientBase
{
    ...
    {
        $body = json_encode(['merchant_reference1' => $oxOrderNr]);
        $oResponse =
            $this->patch(
    ...
    protected function handleResponse(\Requests_Response $oResponse, $class, $method)
    {
        ...
        if ($oResponse->status_code == 403) {
            ...
            if ($aResponse['error_code'] === "CAPTURE_NOT_ALLOWED") {
                throw new KlarnaCaptureNotAllowedException(
                    sprintf($message, $this->formatErrorMessage($oResponse)), 403
                );
            } else {
        ...
        if (in_array($oResponse->status_code, $errorCodes)) {
            throw new KlarnaClientException(
                sprintf($message, $this->formatAndShowErrorMessage($oResponse)),
