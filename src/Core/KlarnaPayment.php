...
class KlarnaPayment extends BaseModel
{
     ...
     */
    public function __construct(Basket $oBasket, User $oUser, $aPost = [])
    {
        ...
        if (!(KlarnaUtils::is_ajax()) && $this->isAuthorized() && count($this->aUpdateData) > 0) {
    ...
    {
        $fieldNamesToCheck = ['country', 'given_name', 'family_name'];
        ...
        foreach ($this->_trackedProperties as $sPropertyName) {
            ...
            if ($this->checkSums[$sPropertyName] !== $currentCheckSum) {
                if ($sPropertyName !== '_sPaymentMethod') {
                    ...
                    if ($this->action !== 'addUserData') { // update selected KP method
    ...
    public function fetchCheckSums()
    {
        ...
        if (!$checkSums) {
            $checkSums = ['_aOrderLines' => false, '_aUserData' => false, '_sPaymentMethod' => false];
        }
