...
class KlarnaBasket extends KlarnaBasket_parent
{
    ...
    public function getKlarnaOrderLines($orderMgmtId = null)
    {
        ...
        foreach ($aItems as $oItem) {
            ...
            [
                ...
                $total_tax_amount,
                $quantity_unit
            ] = KlarnaUtils::calculateOrderAmountsPricesAndTaxes($oItem, $orderMgmtId);
            ...
            if (KlarnaUtils::getShopConfVar('blKlarnaEnableAnonymization') === false) {
                ...
                $aProcessedItem['product_identifiers'] = [
        ...
        if (($oWrappingCost && $oWrappingCost->getPrice())) {
            ...
            $this->klarnaOrderLines[] = [
                ...
                'name' => html_entity_decode(
                    Registry::getLang()->translateString('TCKLARNA_GIFT_WRAPPING_TITLE', $iLang),
                    ENT_QUOTES
                ),
                'quantity' => 1,
    ...
    protected function addGiftCardProducts($iLang = null)
    {
        ...
        if (($oGiftCardCost && $oGiftCardCost->getPrice())) {
            ...
            $this->klarnaOrderLines[] = [
                ...
                'name' => html_entity_decode(
                    Registry::getLang()->translateString('TCKLARNA_GIFT_CARD_TITLE', $iLang),
                    ENT_QUOTES
                ),
                'quantity' => 1,
                'total_amount' => $unit_price,
                'total_discount_amount' => 0,
                'total_tax_amount' => KlarnaUtils::parseFloatAsInt(
                    $unit_price - round($unit_price / ($tax_rate / 10000 + 1), 0)
                ),
                'unit_price' => $unit_price,
                'tax_rate' => $tax_rate,
            ];
                ...
                if (!empty($klarnaDiscounts)) {
                    foreach ($klarnaDiscounts as $discount) {
                        foreach ($taxInfo as $taxRate => $unitPrice) {
                            $this->klarnaOrderLines[] = $this->getKlarnaCheckoutDiscount(
                                $oDiscount,
                                $iLang,
    ...
    protected function getKlarnaCheckoutVoucherDiscount(Price $oPrice, $iLang = null)
    {
        ...
        $aItem = [
            ...
            'name' => html_entity_decode(
                Registry::getLang()->translateString('TCKLARNA_VOUCHER_DISCOUNT', $iLang),
                ENT_QUOTES
            ),
            'quantity' => 1,
            'total_amount' => $unit_price,
    ...
    protected function getKlarnaCheckoutDiscount(Price $oPrice, $iLang = null, $taxInfo = null)
    {
        ...
        if ($value < 0) {
            ...
            $name = html_entity_decode(
                Registry::getLang()->translateString('TCKLARNA_SURCHARGE_TITLE', $iLang),
                ENT_QUOTES
            );
    ...
    {
        return KlarnaUtils::isKlarnaPaymentsEnabled() ? $this->tcklarna_calculateDeliveryCost(
        ) : parent::calcDeliveryCost();
    }
    ...
    protected function addKlarnaAnonymousMapping($artOxid, $anonArtNum)
    {
        ...
        $oQueryBuilder
            ...
            ->where('tcklarna_artnum = :tcklarna_artnum')
            ->setParameter(':tcklarna_artnum', $anonArtNum)
            ->andWhere('oxartid = :oxartid')
            ->setParameter(':oxartid', $artOxid);
        $aSelectResult = $oQueryBuilder->execute();
        $aSelectResult = $aSelectResult->fetchAllAssociative();

        if (!$aSelectResult || $aSelectResult == null || empty($aSelectResult) || count($aSelectResult) <= 0) {
            $oQueryBuilder = $oQueryBuilderFactory->create();
            $oQueryBuilder
                ->insert('tcklarna_anon_lookup')
                ->values([
                    'tcklarna_artnum' => ':tcklarna_artnum',
                    'oxartid' => ':oxartid',
                ])
                ->setParameters([
                    ':tcklarna_artnum' => $anonArtNum,
                    ':oxartid' => $artOxid
                ]);
